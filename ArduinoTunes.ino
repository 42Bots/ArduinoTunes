...
// Array with the note durations: a querter not has a duration of 4, half note 2 etc.
...
void playTune(int notes[], int durations[], int BPM) 
{
  ...
  for (int thisNote = 0; thisNote < tuneSize; thisNote++) {
    ...
    // For details on calculating the note duration use the tempo and the note type,
    // see http://bradthemad.org/guitar/tempo_explanation.php.
    // A quarter note at 60 BPM lasts exactly one second.
